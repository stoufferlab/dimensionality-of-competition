...
pdf(
	here::here('figures/Supplementary-Notes/macarthur_IRO.pdf'),
	width=4.45,
	height=3.77
)
...
title_exp <- expression("Identical "*italic(theta)*"; Random "*italic(W)) #*"; Orthogonal "*italic(C))
...
# 	)
# ) %>% as_tibble()
# points(plot_data$x, plot_data$y, col='red')
...
sapply(
	...
	function(i,res.list,res){
		text(
			res[[res.list[i]]]$w_var,
			ifelse(res[[res.list[i]]]$d_var_exp[1]>0.95, 1, 0),
			letters[i],
			cex = 1.5,
			xpd = NA
		)
...
pdf(
	here::here('figures/Supplementary-Notes/macarthur_IRO_mats.pdf'),
	width=6.45,
	height=6.45
)
