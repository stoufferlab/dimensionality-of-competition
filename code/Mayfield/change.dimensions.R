...
change.dimensions <- function(par, focals, competitors, olddimensions, newdimensions){
    if(olddimensions == newdimensions){
        return(par)
    }else{
        # old intercepts
        intercepts.old <- par[seq.int(1,length(focals))]

        # turn the linear version of the response traits into a species by trait matrix
        response.traits.old <- par[seq.int(length(intercepts.old)+1,length(intercepts.old)+length(focals)*olddimensions)]
        ...
        if(olddimensions > newdimensions){
            response.traits.new <- response.traits.old[seq.int(1, length(focals)*newdimensions)]
            effect.traits.new <- effect.traits.old[seq.int(1, (length(competitors))*newdimensions)]
        }else{
            response.traits.new <- c(response.traits.old, rep(0, length(focals)*(newdimensions - olddimensions)))
