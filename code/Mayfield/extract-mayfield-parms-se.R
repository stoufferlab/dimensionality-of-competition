...
for(which.treatment in c('Open','Shade')){
	...
	load(paste0("../../results/Mayfield/mayfield.",which.treatment,".best.Rdata"))
	d <- length(Mayfield.best$weights)
	...
	best.fits <- sapply(
		saved.fits,
		function(x){
			q <- try(load(paste0('../../results/Mayfield/',x)))
	...
	common.competitors <- colSums(mayfield[,competitors])>=3
	...
	write.table(
		...
		paste0("../../results/Mayfield/mayfield.",which.treatment,".lambdas.csv"),
		quote=FALSE,
		col.names=TRUE,
		sep=" ",
		row.names=TRUE
	...
	for(i in 1:d){
		write.table(
			...
			paste0("../../results/Mayfield/mayfield.",which.treatment,".response.",i,".csv"),
			quote=FALSE,
			col.names=TRUE,
			sep=" ",
			row.names=TRUE
		...
		write.table(
			...
			paste0("../../results/Mayfield/mayfield.",which.treatment,".effect.",i,".csv"),
			quote=FALSE,
			col.names=TRUE,
			sep=" ",
			row.names=TRUE
	...
	write.table(
		...
		paste0("../../results/Mayfield/mayfield.",which.treatment,".alphas.csv"),
		quote=FALSE,
		col.names=TRUE,
		sep=" ",
		row.names=TRUE
