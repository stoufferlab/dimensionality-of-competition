...
change.dimensions <- function(par, targets, competitors, olddimensions, newdimensions){
    if(olddimensions == newdimensions){
        ...
        effect.traits.old <- par[seq.int(length(intercepts.old)+length(response.traits.old)+1, length(intercepts.old)+length(response.traits.old)+(length(competitors)-1)*olddimensions)]

        if(olddimensions > newdimensions){
            response.traits.new <- response.traits.old[seq.int(1, length(targets)*newdimensions)]
            effect.traits.new <- effect.traits.old[seq.int(1, (length(competitors)-1)*newdimensions)]
        }else{
            response.traits.new <- c(response.traits.old, rep(0, length(targets)*(newdimensions - olddimensions)))
            effect.traits.new <- c(effect.traits.old, rep(0,(length(competitors)-1)*(newdimensions - olddimensions)))
        }
