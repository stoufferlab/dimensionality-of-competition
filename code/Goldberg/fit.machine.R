...
if(TRUE){
	...
	null.formula <- as.formula("biomass ~ 0 + target")
	null.fit <- glm(
		null.formula,
		family=which.family,
		data=goldberg2,
	...
	model.formula <- as.formula(paste0("biomass ~ 0 + target + ",paste0("target:",competitors,collapse=" + ")))
	x <- model.matrix(model.formula, goldberg2)
	y <- goldberg2$biomass
	...
	targets <- levels(goldberg2$target)
	...
	for(dimensions in seq.int(1,7)){ #seq.int(1,2)){
		...
		for(attempts in c(1,3)){ #seq.int(3)){
			...
			if(is.na(poop)){
				...
				lower <- c(
					rep(0, nlevels(goldberg2$target)),
					rep(0, nlevels(goldberg2$target) * dimensions),
					rep(-2, nlevels(goldberg2$target) * dimensions)
				)
				upper <- c(
					rep(Inf, nlevels(goldberg2$target)),
