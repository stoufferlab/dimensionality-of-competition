...
for(which.treatment in c('Open','Shade')){
	...
	all.fits <- lapply(
		...
		function(d){
			...
			aics <- sapply(
				saved.fits,
				function(x){
					q <- try(load(paste0('../../results/Australia/',x)))
					# print(paste0('../../results/Australia/',x))
	...
	write.table(
		all.fits,
		file=paste0("../../results/Australia/australia.",which.treatment,".fit.summary.csv")
	)
	...
	write.table(
		alphas,
		paste0("../../results/Australia/australia.",which.treatment,".alphas.regression.csv")
	)
	...
	load(paste0('../../results/Australia/',rownames(subset(all.fits, dimensions==max(all.fits$dimensions))[which.fit,])))
	assign("y", eval(parse(text = paste0(which.treatment,".optim.lowD"))))
	refp <- response.effect.from.pars(
		y@coef,
		targets,
	...
	write.table(
		refp$alphas,
		paste0("../../results/Australia/australia.",which.treatment,".full.alphas.fit.csv")
	)
	# R^2
	write.table(
			cbind(
	...
	for(d in seq.int(max(all.fits$dimensions))){
		...
		write.table(
			alphas.d,
			paste0("../../results/Australia/australia.",which.treatment,".full.alphas.",d,".fit.csv")
		)
	}
	...
	load(paste0('../../results/Australia/',rownames(subset(all.fits, dimensions==which.dim)[which.fit,])))
	assign("y", eval(parse(text = paste0(which.treatment,".optim.lowD"))))
	refp <- response.effect.from.pars(
		y@coef,
		targets,
	...
	write.table(
		refp$alphas,
		paste0("../../results/Australia/australia.",which.treatment,".D",which.dim,".alphas.fit.csv")
	)
